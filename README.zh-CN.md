...
- **组织架构模块**：单独展示、记录、编辑和存储部门、岗位、人员和正式汇报线；
...
Demo 里有独立的 **组织架构** tab，用来管理相对稳定的组织事实：
...
- **输入工作台**：粘贴职场上下文、导入组织架构图、迁移旧记忆；
- **设置中心**：配置 API 地址、session、记忆策略、模型 provider、模型参数、聊天模板和自动切换；
- **结果区**：关系图、风险、隐藏假设、工作内容网、组织架构、知识框架和可点击的人物档案。
...
| `org_structure` | 独立组织架构模块：部门、人员、岗位、正式汇报线、部门树和人员汇报树。 |
