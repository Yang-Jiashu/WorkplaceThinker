...
from graphcore.coregraph.exceptions import (
    PipelineCancelledException,
    ChunkTokenLimitExceededError,
)
from graphcore.coregraph.utils import (
    logger,
    compute_mdhash_id,
    Tokenizer,
    is_float_regex,
...
from graphcore.coregraph.base import (
    BaseGraphStorage,
    BaseKVStorage,
    BaseVectorStorage,
    TextChunkSchema,
...
from graphcore.coregraph.prompt import PROMPTS
from graphcore.coregraph.constants import (
    GRAPH_FIELD_SEP,
...
try:
    from docthinker.bm25_hybrid import bm25_retriever, normalize_bm25_scores
...
def _merge_candidates_by_key(
    ...
    *,
    ...
    """Merge two lists of candidates without duplicates, preserving order."""
    if limit <= 0:
        return []
    merged: list[dict] = []
    seen: set[str] = set()
...
async def _get_vector_context(
    ...
    """
    strategy = getattr(query_param, "chunk_retrieval", "hybrid")
    search_top_k = query_param.chunk_top_k or query_param.top_k
    bm25_chunks: list[dict] = []

    if (
        _BM25_AVAILABLE
        and strategy in {"bm25", "hybrid"}
        and text_chunks_db is not None
    ):
        bm25_raw = bm25_retriever.search_chunks(text_chunks_db, query, search_top_k)
        for entry, _ in bm25_raw:
            payload = entry.payload
            bm25_chunks.append(
                {
                    "content": payload.get("content", ""),
                    "created_at": payload.get("created_at"),
                    "file_path": payload.get("file_path", "unknown_source"),
                    "source_type": "bm25",
                    "chunk_id": payload.get("chunk_id", entry.doc_id),
                }
            )
        if bm25_chunks:
            logger.info(
                "BM25 chunk retrieval returned %d results (strategy=%s)",
                len(bm25_chunks),
                strategy,
            )
    ...
    if strategy == "bm25" and len(bm25_chunks) >= search_top_k:
        use_vector = False
            ...
            if not results:
                ...
                for result in results:
                    if "content" in result:
                        ...
                        vector_chunks.append(chunk_with_metadata)
                logger.info(
                    f"Naive query: {len(vector_chunks)} chunks (chunk_top_k:{search_top_k} cosine:{cosine_threshold})"
                )
        except Exception as e:  # pragma: no cover - log then fallback
            logger.error(f"Error in _get_vector_context: {e}")

    merged = _merge_candidates_by_key(
        bm25_chunks,
        vector_chunks,
        key="chunk_id",
        limit=search_top_k,
...
async def _expand_search_by_graph_traversal(
    ...
    """
    Expand retrieval by BFS graph traversal from seed entities.
    Returns (new_entity_names, new_edge_pairs) not already in seeds.
    """
    if max_hops <= 0:
        return [], []
    visited: set[str] = set(seed_entity_names)
    frontier = list(seed_entity_names)
    ...
    for _ in range(max_hops):
        ...
        frontier = next_frontier[:max_expand_entities]
    new_entity_names = [n for n in visited if n not in seed_entity_names][:max_expand_entities]
    new_edge_pairs = [p for p in all_edge_pairs if p not in seed_edge_pairs]
    return new_entity_names, new_edge_pairs
...
async def _perform_kg_search(
    ...
    When graph_traversal_hops > 0, expands results by BFS along graph edges.
    """
    ...
    # Graph traversal expansion: expand along edges (BFS) when graph_traversal_hops > 0
    traversal_hops = getattr(query_param, "graph_traversal_hops", 0)
    if traversal_hops > 0 and (final_entities or final_relations):
        seed_entity_names = {e.get("entity_name") for e in final_entities if e.get("entity_name")}
        for r in final_relations:
            src = r.get("src_id") or (r.get("src_tgt") or (None, None))[0]
            tgt = r.get("tgt_id") or (r.get("src_tgt") or (None, None))[1]
            if src:
                seed_entity_names.add(src)
            if tgt:
                seed_entity_names.add(tgt)
        seed_edge_pairs = set()
        for r in final_relations:
            if "src_tgt" in r:
                seed_edge_pairs.add(tuple(sorted(r["src_tgt"])))
            else:
                s, t = r.get("src_id"), r.get("tgt_id")
                if s and t:
                    seed_edge_pairs.add(tuple(sorted([s, t])))
        new_entity_names, new_edge_pairs = await _expand_search_by_graph_traversal(
            seed_entity_names,
            seed_edge_pairs,
            knowledge_graph_inst,
        ...
        if new_entity_names or new_edge_pairs:
            if new_entity_names:
                nodes_dict = await knowledge_graph_inst.get_nodes_batch(new_entity_names)
                degrees_dict = await knowledge_graph_inst.node_degrees_batch(new_entity_names)
            ...
            if new_edge_pairs:
                edge_pairs_dicts = [{"src": p[0], "tgt": p[1]} for p in new_edge_pairs]
                edge_data_dict = await knowledge_graph_inst.get_edges_batch(edge_pairs_dicts)
                edge_degrees_dict = await knowledge_graph_inst.edge_degrees_batch(
                    list(new_edge_pairs)
                )
                for pair in new_edge_pairs:
            ...
            logger.info(
                f"Graph traversal (+{traversal_hops} hop(s)): +{len(new_entity_names)} entities, +{len(new_edge_pairs)} relations"
            )
...
async def _build_context_str(
    ...
    # Report token usage for visibility during testing
    try:
        entity_tokens = len(tokenizer.encode(entities_str))
        relation_tokens = len(tokenizer.encode(relations_str))
        chunk_tokens = len(tokenizer.encode(text_units_str))
        ...
        logger.info(
            "Context tokens -> entities:%d relations:%d chunks:%d total:%d (max_total:%d)",
            entity_tokens,
            relation_tokens,
            chunk_tokens,
            total_tokens,
        ...
        logger.debug("Failed to compute token stats for context")

    # Persist token stats and full context independently to avoid losing dumps
    try:
        log_path = os.getenv("TOKEN_STATS_LOG", "token_stats.jsonl")
        Path(log_path).parent.mkdir(parents=True, exist_ok=True)
        if stats is not None:
            with Path(log_path).open("a", encoding="utf-8") as f:
                f.write(json.dumps(stats, ensure_ascii=False) + "\n")
        dump_dir = Path(os.getenv("CONTEXT_DUMP_DIR", "context_dumps"))
        ...
        dump_path = dump_dir / f"context_{dump_payload['id']}.json"
        dump_path.write_text(json.dumps(dump_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        logger.debug("Failed to persist context/token stats")
...
async def _get_node_data(
    ...
    strategy = getattr(query_param, "entity_retrieval", "hybrid")
    bm25_candidates: list[dict] = []

    if _BM25_AVAILABLE and strategy in {"bm25", "hybrid"}:
        bm25_raw = bm25_retriever.search_entities(
            knowledge_graph_inst, query, query_param.top_k
        )
        score_map = normalize_bm25_scores(bm25_raw)
        for entry, score in bm25_raw:
            entity_name = entry.payload.get("entity_name", entry.doc_id)
            bm25_candidates.append(
                {
                    "entity_name": entity_name,
                    "distance": 1 - score_map.get(entry.doc_id, 0.0),
                    "created_at": entry.payload.get("created_at"),
                    "node_data": entry.payload,
    ...
    node_ids = [cand["entity_name"] for cand in combined_candidates]
    ...
    for candidate in combined_candidates:
        ...
        node_data = candidate.get("node_data") or nodes_dict.get(entity_name)
        if node_data is None:
            logger.warning("Missing node data for %s", entity_name)
            continue
        node_datas.append(
            {
                **node_data,
                "entity_name": entity_name,
                "rank": degrees_dict.get(entity_name, 0),
...
async def _get_edge_data(
    ...
    strategy = getattr(query_param, "relation_retrieval", "hybrid")
    bm25_candidates: list[dict] = []
    if _BM25_AVAILABLE and strategy in {"bm25", "hybrid"}:
        bm25_raw = bm25_retriever.search_relations(
            knowledge_graph_inst, keywords, query_param.top_k
        )
        score_map = normalize_bm25_scores(bm25_raw)
        for entry, score in bm25_raw:
            payload = entry.payload
            src = payload.get("src_id") or payload.get("source") or ""
            tgt = payload.get("tgt_id") or payload.get("target") or ""
            bm25_candidates.append(
                {
                    "src_id": src,
                    "tgt_id": tgt,
    ...
    combined_candidates = _merge_candidates_by_key(
        [{**c, "relation_key": _relation_key(c)} for c in bm25_candidates],
        [{**c, "relation_key": _relation_key(c)} for c in vector_candidates],
        key="relation_key",
        limit=query_param.top_k,
    )
    ...
    edge_pairs_dicts = [
        {"src": cand.get("src_id"), "tgt": cand.get("tgt_id")}
