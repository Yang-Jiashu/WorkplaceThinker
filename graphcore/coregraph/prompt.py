...
1. SPARQL Chain-of-Thought Reasoning:
  Before generating the final answer, internally decompose the user query into a SPARQL-like chain of triple patterns to systematically retrieve and bind information from the Knowledge Graph.
  - Express each information need as a triple pattern: `(?variable, relationship, ?variable_or_entity)`
  - Chain patterns together via shared variables to form a reasoning path
  - Bind variables to concrete values found in the Context
  - Use the completed variable binding table to construct the final answer

  Example — Query: "Who are the children of the protagonist and what happened to them?"
  ```
  (?protagonist, is_a, "main character")           → ?protagonist = "人祖"
  (?child, is_child_of, ?protagonist)              → ?child = {{"太日阳莽", "古月阴荒", "北冥冰魄", ...}}
  (?child, experienced_event, ?event)              → ?event = {{"被困平凡深渊", "踏上成败山", ...}}
  (?child, has_outcome, ?outcome)                  → ?outcome = {{"获得自由", "化为生命蛊", ...}}
  ```

  This reasoning process should guide your information extraction but does NOT need to be shown in the output. Only output the final answer.
