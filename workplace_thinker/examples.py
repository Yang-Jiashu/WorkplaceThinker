...
async def example_new_employee_journey():
    ...
    print(f"   图谱节点数: {stats1.get('people_count', 0)}")
    print(f"   图谱边数: {stats1.get('relationships_count', 0)}")
    
    if 'persistent_graph' in result1:
        graph = result1['persistent_graph']
        print(f"\n🌐 当前图谱（第1天）：")
        print(f"   人物节点: {[n['label'] for n in graph['nodes']]}")
    ...
    # 查看图谱增长情况
    stats2 = harness.get_memory_stats()
    print(f"\n💾 记忆状态（第2天后）：")
    print(f"   图谱节点数: {stats2.get('people_count', 0)}")
    print(f"   图谱边数: {stats2.get('relationships_count', 0)}")
    print(f"   快照数: {stats2.get('snapshots_count', 0)}")
    
    if 'graph_timeline' in result2:
        print(f"\n⏱️  图谱时间线：")
        for i, entry in enumerate(result2['graph_timeline']):
            print(f"   [{i}] {time.strftime('%H:%M:%S', time.localtime(entry['timestamp']))} - "
                  f"{entry['people_count']}人, {entry['relationships_count']}边")
    ...
    # 查看最终的完整图谱
    stats3 = harness.get_memory_stats()
    print(f"\n💾 记忆状态（第3天后）：")
    print(f"   图谱节点数: {stats3.get('people_count', 0)}")
    print(f"   图谱边数: {stats3.get('relationships_count', 0)}")
    print(f"   快照数: {stats3.get('snapshots_count', 0)}")
    
    final_graph = harness.get_current_graph()
    if final_graph:
        print(f"\n🌐 最终图谱（完整关系）：")
        print(f"   人物: {[n['label'] for n in final_graph['nodes']]}")
        print(f"   关系: ")
    ...
    if memory_export:
        ...
        print(f"   图谱数据: {len(memory_export['people'])}人, {len(memory_export['relationships'])}边")
    ...
    print("   3. 持续构建关系图谱，不是每次都重新分析")
    print("   4. 支持时间线，查看关系如何演变")
    print("   5. 提供中立、证据为基础的建议，而不是阴谋论")
    print("   6. 推荐具体的确认问题，帮助新人保护自己")
