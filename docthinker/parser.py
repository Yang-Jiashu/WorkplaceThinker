...
class MineruParser(Parser):
    ...
    def parse_text_file(
        ...
        **kwargs,
        ...
        """
        Parse text file (.txt, .md) directly without converting to PDF or using MinerU.
        This is much faster and avoids model download issues for plain text.

        Supported formats: .txt, .md

        Args:
            text_path: Path to the text file (.txt, .md)
            output_dir: Output directory path (unused, but kept for signature compatibility)
            lang: Document language (unused, but kept for signature compatibility)
            **kwargs: Additional parameters (unused, but kept for signature compatibility)
        ...
        try:
            ...
            for encoding in encodings:
                try:
                    with open(text_path, "r", encoding=encoding) as f:
                        text_content = f.read()
                    logging.info(f"Successfully read file {text_path.name} with {encoding} encoding")
                    break
                except UnicodeDecodeError:
                    continue
            else:
            ...
            # Return as a single text block compatible with MinerU schema
            return [{
                "type": "text",
                "text": text_content,
